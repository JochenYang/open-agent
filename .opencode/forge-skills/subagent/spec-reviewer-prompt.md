...
Dispatch a spec-compliance reviewer as a `general` subagent via the `task` tool,
...
Dispatch the same reviewer again as a `general` subagent via the `task` tool,
...
this (see `forge:subagent`): the task is not complete while any in-scope claim is
