...
**Save reports to:** `docs/forge/reports/<feature-name>.md`
...
8. **Commit and transition** — commit report + markers, invoke forge:merge
...
status: delivered
specs:
  - docs/forge/specs/<spec-1>.md
  - docs/forge/specs/<spec-2>.md
plans:
  - docs/forge/plans/<plan>.md
branch: <branch-name>
...
**Preceded by:** forge:verify (feature must be verified before documenting)
**Followed by:** forge:merge (the terminal workflow step)

After saving the report and marking specs, offer transition:

> "Final report written and committed to `<path>`. Related specs marked. Ready to finish the branch — invoking forge:merge."
