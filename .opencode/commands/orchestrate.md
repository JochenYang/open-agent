...
1. **Primary 代理**（谋士、行者）作为会话主导者，持续存在于整个会话中
2. **Subagent 代理**（探路、御史、明镜、校验、工匠等）通过 `task` 工具调用
3. **`task` 工具** 为子代理创建独立上下文，执行后返回结果到主会话
4. **自定义命令** 位于 `.opencode/commands/`，定义可复用的工作流模板

与 Claude Code 的关键区别：
- OpenCode 的 `task` 工具对等 Claude Code 的 `Agent` 工具
- OpenCode primary 代理可持久存在于会话，编排更自然（无需每次重新加载代理定义）
...
| 代理 | 文件 | 模式 | 职责 |
|------|------|------|------|
| 谋士 | 谋士.md | primary | 需求分析、方案规划、风险识别 |
| 行者 | 行者.md | primary | 主执行者、任务调度、结果交付 |
...
| --- | --- | --- | --- | --- |
| `feature` | `谋士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 谋士之后 | 行者/工匠 | 御史 |
| `feature-tdd` | `谋士 -> 校验 -> 御史` `↺ 修复循环` | 谋士之后 | 校验 | 御史 |
| `bugfix` | `明镜 -> 校验 -> 御史` `↺ 修复循环` | 无（除非分析改变范围） | 校验 | 御史 |
| `refactor` | `谋士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 谋士之后 | 行者/工匠 | 御史 |
| `ui-design` | `谋士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 谋士之后 | 行者/工匠 | 御史 |
| `secure-feature` | `谋士 -> 行者+工匠 -> 刺史 -> 御史` `↺ 专长+审查循环` | 谋士之后 | 行者/工匠 | 刺史 -> 御史 |
| `db-feature` | `谋士 -> 匠作 -> 行者+工匠 -> 御史` `↺ 专长+审查循环` | 谋士之后 | 匠作（schema/data 问题），否则行者/工匠 | 匠作（schema/data 变更后），然后御史 |
| `performance-audit` | `明镜 -> 斥候 -> 行者+工匠 -> 御史` `↺ 专长+审查循环` | 无（除非优化范围显著变化） | 斥候（测量问题），否则行者/工匠 | 斥候（指标/分析变更后），然后御史 |
| `deploy` | `谋士 -> 都尉 -> 御史` `↺ 专长+审查循环` | 谋士之后 | 都尉 | 都尉（基础设施/流水线变更后），然后御史 |
