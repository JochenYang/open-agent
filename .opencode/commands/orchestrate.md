...
| 斥候 | 斥候.md | subagent | 性能瓶颈定位、指标分析、优化方案        |
...
本批任务（按发出顺序执行）:
  - task @工匠: 改模块A / 目标文件 path/to/a.ts
  - task @工匠: 改模块B / 目标文件 path/to/b.ts
写入冲突预检: ✅ 无重叠
...
- 对写入型 task（工匠/匠作）用 `background: true` 做真正并发（无 write-lock）
- 同一轮调度御史和工匠处理同一文件（读写冲突）
