...
1. **Primary 代理**（谋士 plan 模式、行者 build 模式）作为会话主导者，手动切换
2. **Subagent 代理**（策士、探路、御史、明镜等）通过行者的 `task` 工具调用
3. **`task` 工具** 为子代理创建独立上下文，执行后返回结果到主会话
4. **自定义命令** 位于 `.opencode/commands/`，定义可复用的工作流模板

双规划机制：
- **谋士**（primary plan 模式）：手动切换，做宏观架构决策和风险权衡
- **策士**（subagent）：行者通过 task 调用，做可执行步骤拆解和子代理分派
...
| 行者 | 行者.md | primary | build 模式，主执行者、任务调度、结果交付 |
...
| --- | --- | --- | --- | --- |
| `feature` | `策士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 策士之后 | 行者/工匠 | 御史 |
| `feature-tdd` | `策士 -> 校验 -> 御史` `↺ 修复循环` | 策士之后 | 校验 | 御史 |
| `bugfix` | `明镜 -> 校验 -> 御史` `↺ 修复循环` | 无（除非分析改变范围） | 校验 | 御史 |
| `refactor` | `策士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 策士之后 | 行者/工匠 | 御史 |
| `ui-design` | `策士 -> 行者+工匠 -> 御史` `↺ 修复循环` | 策士之后 | 行者/工匠 | 御史 |
| `secure-feature` | `策士 -> 行者+工匠 -> 刺史 -> 御史` `↺ 专长+审查循环` | 策士之后 | 行者/工匠 | 刺史 -> 御史 |
| `db-feature` | `策士 -> 匠作 -> 行者+工匠 -> 御史` `↺ 专长+审查循环` | 策士之后 | 匠作（schema/data 问题），否则行者/工匠 | 匠作（schema/data 变更后），然后御史 |
